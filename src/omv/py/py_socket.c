 ...
 * Modified by wangxhere@gmail.com for use with lwip
...
#define MAX_RX_PACKET       (MEM_SIZE / 4)
#define MAX_TX_PACKET       (MEM_SIZE / 4)
#define min(a,b) \
   ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
     _a < _b ? _a : _b; })
...
static mp_obj_t socket_bind(mp_obj_t self_in, mp_obj_t addr_obj)
{
    ...
    /* bind socket */
    if (bind(self->fd, (struct sockaddr*) &addr_in,
...
//    wlan_clear_fd_state(fd);
    ...
    cli_addr->items[1] =
    		mp_obj_new_int(((struct sockaddr_in*)&addr)->sin_port);
...
static mp_obj_t socket_settimeout(mp_obj_t self_in, mp_obj_t timeout)
{
    ...
    if (setsockopt(self->fd, SOL_SOCKET, SO_RCVTIMEO, &optval, optlen) != 0) {
